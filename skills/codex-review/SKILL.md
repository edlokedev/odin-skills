---
name: codex-review
description: Adversarial PLAN-review loop — Claude drafts the plan into PLAN.md, OpenAI Codex reviews it read-only and returns VERDICT:APPROVED/REVISE, Claude revises against the same Codex session until APPROVED or MAX_ROUNDS, then the human signs off before any code. Use when you already have a plan and want a cross-model stress-test, especially for high-stakes work (auth, schema, concurrency, migrations, payments). Triggers: "/codex-review", "codex review my plan", "argue this plan with Codex". For a requirements interview first use /grill-me-codex; not for reviewing already-written code (use /codex:review) and not for trivial changes.
---
